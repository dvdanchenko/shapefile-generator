...
"""
Done. Stage 19 — final UX, performance and release hardening is complete.

Changes
Escape cancels Create Mode
Esc now cancels interactive creation cleanly.
Clear QC Canvas consistency
Clearing the canvas while Create Mode is active now also exits Create Mode and resets its controls.
Folder loading improved
.shp, .SHP, .Shp, etc. are all detected.
Batch loading summary improved
Shows which files were skipped instead of only giving a number.
Still avoids interrupting the user with a dialog for every failed file.
Shutdown cleanup
Stops LOD timers and exits Create Mode cleanly when closing the application.
Does not unnecessarily block the UI waiting for background workers.
Existing performance architecture retained
Cached LOD rendering.
Background geometry processing/QC/indexing.
No geometry rebuilding during pan/zoom.
Existing functionality preserved
Points / Line / Polygon creation.
...
I also syntax-checked the Stage 19 main file and both supporting modules successfully.

Final Stage 19 file

Download PipelineRouteCreator_stage19.py

This is now the release-candidate version of the staged development path.
"""
import sys
import time
...
from PySide6.QtWidgets import (
    ...
    QTableView, QAbstractItemView, QLineEdit, QHeaderView, QSpinBox, QTabWidget
)
from PySide6.QtCore import (
    Qt, Signal, QThreadPool, QTimer, QSettings, QEvent
)
...
def make_layer_icon(color_hex: str, status: str) -> QIcon:
    """Create a combined layer-colour swatch and QC status icon."""
    status = status.upper()
    status_color = {
        "PASS": "#2e7d32",
        "WARNING": "#f9a825",
    ...
    painter.drawRect(1, 2, 10, 10)
    painter.setPen(Qt.NoPen)
    painter.setBrush(QColor(status_color))
    painter.drawEllipse(16, 3, 8, 8)
    painter.end()
    return QIcon(pixmap)
...
class QCMapCanvas(pg.PlotWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Stage 17: attribute-driven map styling.
        # These must be initialized here, before any layer visibility or
        # styling operation can occur.
        self._attribute_style_items = []
        self._attribute_style_aux_items = []
        self._attribute_style_layer_id = None
        self._attribute_style_field = None
        self._attribute_style_mode = None
    ...
    def _switch_visible_lod(self):
        for layer in self.loaded_layers.values():
            ...
            # Attribute styling replaces the base graphics.  LOD switching
            # must never make the original one-colour layer visible again.
            if layer.get("attribute_style") and self._attribute_style_layer_id in self.loaded_layers:
                if self.loaded_layers.get(self._attribute_style_layer_id) is layer:
                    continue
    ...
    def clear_attribute_style(self, layer_id=None):
        """Remove attribute-driven graphics and restore normal layer graphics."""
        if layer_id is not None and self._attribute_style_layer_id != layer_id:
            return

        # Attribute-style auxiliary items such as LegendItem and ColorBarItem
        # are parented to the PlotItem rather than behaving like ordinary
        # PlotDataItems.  Removing them only through PlotItem.removeItem()
        # can leave the parent-owned graphics object visible.  Detach them
        # from the PlotItem explicitly first, then remove ordinary items.
        aux_items = list(getattr(self, "_attribute_style_aux_items", []))
        style_items = list(getattr(self, "_attribute_style_items", []))

        for item in aux_items:
            try:
                if item.parentItem() is self.getPlotItem():
        ...
        styled_layer_id = getattr(self, "_attribute_style_layer_id", None)
        self._attribute_style_items = []
        self._attribute_style_aux_items = []
        self._attribute_style_layer_id = None
        self._attribute_style_field = None
        ...
        if styled_layer_id is not None:
            layer = self.loaded_layers.get(styled_layer_id)
            if layer is not None:
                layer["attribute_style"] = None
                self._set_base_layer_graphics_visible(layer, layer.get("visible", True))
                self._switch_visible_lod()

    def _set_base_layer_graphics_visible(self, layer, visible):
        items = list(layer.get("items", []))
        items.extend(item for item in layer.get("lod_items", {}).values() if item is not None)
    ...
    @staticmethod
    def _attribute_is_numeric(series):
        try:
            from pandas.api.types import is_numeric_dtype
            return bool(is_numeric_dtype(series))
        except Exception:
            try:
                np.asarray(series.dropna(), dtype=float)
                return True
            except Exception:
                return False

    @staticmethod
    def _attribute_value_text(value):
        if value is None:
            return "<NULL>"
        try:
            ...
            pass
        try:
            if hasattr(value, "__float__") and np.isnan(float(value)):
                return "<NULL>"
        except Exception:
            pass
        text = str(value)
    ...
    def _render_attribute_style(self, layer_id, field, mode, vmin=None, vmax=None, max_categories=16):
        layer = self.loaded_layers.get(layer_id)
        if layer is None:
            return False
        ...
        self.clear_attribute_style()
        self._set_base_layer_graphics_visible(layer, False)

        series = gdf[field]
        numeric = self._attribute_is_numeric(series)
        if mode == "Auto":
            mode = "Numerical" if numeric else "Categorical"
        if mode == "Numerical" and not numeric:
            QMessageBox.warning(self, "Attribute Styling", f"Field '{field}' is not numeric.")
            self._set_base_layer_graphics_visible(layer, True)
            return False
        ...
        if mode == "Numerical":
            try:
                ...
                QMessageBox.warning(self, "Attribute Styling", f"Field '{field}' cannot be interpreted as numeric data.")
                self._set_base_layer_graphics_visible(layer, True)
                return False
            finite = values[np.isfinite(values)]
            if finite.size == 0:
                QMessageBox.warning(self, "Attribute Styling", f"Field '{field}' contains no finite numeric values.")
                self._set_base_layer_graphics_visible(layer, True)
                return False
            try:
                low = float(vmin)
            ...
            # 24 bins give a visually continuous ramp while keeping the
            # number of graphics items bounded for large layers.
            bin_count = 24
            ...
            for b in range(bin_count):
                ...
                mask &= values >= edges[b]
                if b == bin_count - 1:
                    mask &= values <= edges[b + 1]
                else:
                    mask &= values < edges[b + 1]
                indices = np.flatnonzero(mask)
                if indices.size == 0:
                    continue
                midpoint = (edges[b] + edges[b + 1]) / 2.0
                t = (midpoint - low) / (high - low)
                color = cmap.map(float(np.clip(t, 0.0, 1.0)), mode="qcolor")
                self._add_attribute_group_graphics(gdf, indices, color, layer.get("name", "Layer"))

            # Nulls are rendered with the normal layer colour.
            null_indices = np.flatnonzero(~np.isfinite(values))
            if null_indices.size:
                self._add_attribute_group_graphics(gdf, null_indices, QColor(layer.get("color", "#808080")), layer.get("name", "Layer"))

            # Add a compact numeric colourbar to the plot. If the installed
            # pyqtgraph version does not support ColorBarItem, the styling
            # remains active and the dialog still reports the numeric range.
            try:
                colorbar = pg.ColorBarItem(values=(low, high), colorMap=cmap, label=field)
                self.addItem(colorbar, ignoreBounds=True)
                self._attribute_style_aux_items.append(colorbar)
            except Exception:
                pass

            layer["attribute_style"] = {
                "field": field, "mode": "Numerical", "min": low, "max": high
            }
            self._attribute_style_layer_id = layer_id
            self._attribute_style_field = field
            self._attribute_style_mode = "Numerical"
            self.status_message.emit(f"Attribute styling: {layer['name']} by '{field}' ({low:,.6g} to {high:,.6g}).")
            return True

        # Categorical styling.
        values = series.to_numpy(dtype=object)
        labels = np.asarray([self._attribute_value_text(v) for v in values], dtype=object)
        unique, counts = np.unique(labels, return_counts=True)
        order = np.argsort(-counts)
        unique = unique[order]
        counts = counts[order]
        limit = max(2, int(max_categories))
        shown = list(unique[:limit])
        if len(unique) > limit:
            shown.append("<Other>")
        ...
        legend = pg.LegendItem(offset=(10, 10), colCount=1)
        legend.setParentItem(self.getPlotItem())
        self._attribute_style_aux_items.append(legend)

        for index, label in enumerate(shown):
            if label == "<Other>":
                mask = ~np.isin(labels, np.asarray(shown[:-1], dtype=object))
            else:
                mask = labels == label
            indices = np.flatnonzero(mask)
            if indices.size == 0:
            ...
            items_before = len(self._attribute_style_items)
            self._add_attribute_group_graphics(gdf, indices, color, layer.get("name", "Layer"))
            if len(self._attribute_style_items) > items_before:
                sample = pg.PlotDataItem([0, 1], [0, 0], pen=pg.mkPen(color=color, width=3))
                legend.addItem(sample, str(label))
                self._attribute_style_aux_items.append(sample)

        layer["attribute_style"] = {
            "field": field, "mode": "Categorical", "categories": shown
        }
        self._attribute_style_layer_id = layer_id
        self._attribute_style_field = field
        self._attribute_style_mode = "Categorical"
        self.status_message.emit(f"Attribute styling: {layer['name']} by '{field}' ({len(shown)} categories shown).")
        return True

    def _add_attribute_group_graphics(self, gdf, indices, color, layer_name):
        subset = gdf.iloc[np.asarray(indices, dtype=int)]
        point_mask = subset.geometry.geom_type.isin(["Point", "MultiPoint"])
        if point_mask.any():
            pts = subset.loc[point_mask, "geometry"].explode(index_parts=False).dropna()
            if not pts.empty:
                coords = shapely.get_coordinates(pts.to_numpy())
                if coords.size:
                    item = pg.ScatterPlotItem(
                        x=coords[:, 0], y=coords[:, 1], size=7,
                        pen=pg.mkPen(color=color, width=1),
                        brush=pg.mkBrush(color=color), name=layer_name,
                    )
                    self.addItem(item)
                    self._attribute_style_items.append(item)

        complex_mask = subset.geometry.geom_type.isin([
            "LineString", "MultiLineString", "Polygon", "MultiPolygon"
        ])
        if complex_mask.any():
            x, y = _build_gis_render_arrays(subset.loc[complex_mask], tolerance=0.0)
            if x.size:
                item = pg.PlotCurveItem(
                    x, y, pen=pg.mkPen(color=color, width=1),
                    connect="finite", antialias=False,
                )
                self.addItem(item)
                self._attribute_style_items.append(item)

    def show_attribute_style_dialog(self, layer_id):
        layer = self.loaded_layers.get(layer_id)
        if layer is None:
            return
        gdf = layer.get("full_gdf")
        ...
        if not fields:
            QMessageBox.information(self, "Attribute Styling", "This layer has no attribute fields to style.")
            return

        dialog = QDialog(self)
        dialog.setWindowTitle(f"Attribute Map Styling — {layer.get('name', 'Layer')}")
        dialog.resize(500, 360)
        form = QFormLayout(dialog)

        field_combo = QComboBox()
        field_combo.addItems(fields)
        form.addRow("Attribute field:", field_combo)

        mode_combo = QComboBox()
        mode_combo.addItems(["Auto", "Numerical", "Categorical"])
        form.addRow("Style type:", mode_combo)

        min_edit = QLineEdit()
        max_edit = QLineEdit()
        form.addRow("Numeric minimum:", min_edit)
        form.addRow("Numeric maximum:", max_edit)

        category_spin = QSpinBox()
        category_spin.setRange(2, 20)
        category_spin.setValue(12)
        form.addRow("Maximum categories:", category_spin)
        ...
        def refresh():
            ...
            numeric = self._attribute_is_numeric(gdf[field])
            if numeric:
                arr = np.asarray(gdf[field], dtype=float)
                finite = arr[np.isfinite(arr)]
                if finite.size:
                    min_edit.setText(f"{np.min(finite):.12g}")
                    max_edit.setText(f"{np.max(finite):.12g}")
                    info.setText(f"Numeric field: {finite.size:,} finite values. Null/non-finite values use the layer colour.")
                else:
                    info.setText("Numeric field, but no finite values are available.")
            else:
                values = [self._attribute_value_text(v) for v in gdf[field].to_numpy(dtype=object)]
                info.setText(f"Categorical field: {len(set(values)):,} unique value(s).")

        field_combo.currentTextChanged.connect(refresh)
        refresh()

        buttons = QHBoxLayout()
        apply_btn = QPushButton("Apply Styling")
        clear_btn = QPushButton("Clear Styling")
        ...
        def apply():
            ...
            if mode == "Auto":
                mode = "Numerical" if self._attribute_is_numeric(gdf[field]) else "Categorical"
            vmin = min_edit.text().strip()
            vmax = max_edit.text().strip()
            ok = self._render_attribute_style(
                layer_id, field, mode,
                vmin=float(vmin) if vmin else None,
                vmax=float(vmax) if vmax else None,
                max_categories=category_spin.value(),
            )
            if ok:
                dialog.accept()

        apply_btn.clicked.connect(apply)
        clear_btn.clicked.connect(lambda: (self.clear_attribute_style(layer_id), dialog.accept()))
    ...
    def _pick_features(self, scene_pos):
        """Return all visible features under/near a map click, nearest first."""
        try:
            ...
            candidates = []
            for layer_order, (layer_id, layer) in enumerate(self.loaded_layers.items()):
                if not layer.get("visible", True):
                    continue
                gdf = layer.get("full_gdf")
                index = layer.get("feature_index")
                if gdf is None or index is None:
                    continue

                try:
                    hits = index.query(search_area, predicate="intersects")
                except TypeError:
                    hits = index.query(search_area)

                for pos in np.asarray(hits, dtype=int):
                    position = int(pos)
                    try:
                    ...
                    contains = bool(is_polygon and geom.contains(click_point))
                    # Containing polygons first, then distance, then layer/feature
                    # order for deterministic results when several features coincide.
                    sort_key = (0 if contains else 1, distance, layer_order, position)
                    candidates.append({
                        "layer_id": layer_id,
                        "position": position,
                        "distance": distance,
    ...
    def _pick_feature(self, scene_pos):
        """Return the best single feature for backward compatibility."""
        candidates = self._pick_features(scene_pos)
        if not candidates:
            return None
        candidate = candidates[0]
        return candidate["layer_id"], candidate["position"], candidate["distance"]
    ...
    def _mouse_clicked(self, event):
        ...
        selected = self._choose_feature_candidate(candidates)
        if selected is None:
            return
    ...
    def _choose_feature_candidate(self, candidates):
        """Let the user choose when multiple visible features overlap a click."""
        # Avoid an impractically large tab bar while still exposing a useful
        # set of nearby/overlapping candidates. Candidates are already sorted
        # by containment, distance and deterministic layer/feature order.
        max_tabs = 40
        shown = candidates[:max_tabs]

        dialog = QDialog(self)
        dialog.setWindowTitle("Select Feature — Multiple Features at Click")
        dialog.resize(760, 560)
        layout = QVBoxLayout(dialog)

        if len(candidates) > max_tabs:
            heading = QLabel(
                f"{len(candidates):,} features are under/near the click. "
                f"Showing the {max_tabs} closest candidates."
            )
        else:
            heading = QLabel(
                f"{len(candidates):,} visible features are under/near the click. "
                "Select the feature you want to inspect."
            )
        ...
        for number, candidate in enumerate(shown, start=1):
            ...
            lines = [
                f"Candidate: {number} of {len(shown)}",
                ...
                f"Click distance: {distance:,.6f}",
            ]
            if candidate["contains"]:
                lines.append("Click relationship: click is inside this polygon")
            ...
            tabs.addTab(page, f"{number}: {layer.get('name', 'Layer')} / F{position + 1}")

        selected_index = [0]

        def preview_candidate(index):
            if not (0 <= index < len(shown)):
                return
            candidate = shown[index]
            selected_index[0] = index
            self._highlight_feature(
                candidate["layer_id"], candidate["position"]
            )

        tabs.currentChanged.connect(preview_candidate)
        preview_candidate(0)

        button_row = QHBoxLayout()
        select_button = QPushButton("Select This Feature")
...
<tbody>{''.join(table_rows)}</tbody>
    ...
    def remove_layer(self, layer_id: str):
        ...
        if self._attribute_style_layer_id == layer_id:
            self.clear_attribute_style(layer_id)
    ...
    def set_layer_visibility(self, layer_id: str, visible: bool):
        ...
        # Attribute styling replaces the base graphics for this layer.
        styled = (
            getattr(self, "_attribute_style_layer_id", None) == layer_id
            and layer.get("attribute_style")
        )
        for item in set(items):
            item.setVisible(visible and not styled)

        if styled:
            for item in getattr(self, "_attribute_style_items", []):
                item.setVisible(visible)
            for item in getattr(self, "_attribute_style_aux_items", []):
                try:
                    item.setVisible(visible)
                except Exception:
                    pass
    ...
    def clear_all_layers(self):
        ...
        # Explicitly remove Stage 17 attribute-style graphics before clearing
        # the plot.  LegendItem/ColorBarItem can be parented to the PlotItem,
        # so relying on PlotWidget.clear() alone can leave auxiliary UI
        # objects behind.
        self.clear_attribute_style()

        # Clearing the scene removes all normal layer graphics. Worker results
        # are ignored after generation changes.
        self.clear()

        # Defensive reset in case an auxiliary graphics item survives the
        # scene clear because it is owned by a parent item.
        self._attribute_style_items = []
        self._attribute_style_aux_items = []
        self._attribute_style_layer_id = None
        self._attribute_style_field = None
...
class PipelineRouteCreator(QMainWindow):
    """Main application window; Stage 19 focuses on release hardening and UX consistency."""
    def __init__(self):
        super().__init__()
        ...
        left_layout.addWidget(QLabel("Active Layers (click colour swatch to change colour):"))
        self.layer_list = QListWidget()
        self.layer_list.itemChanged.connect(self.on_layer_item_changed)
        # Intercept clicks so the colour swatch can act independently from
        # the item's checkbox and selection behaviour.
        ...
        # Escape is a quick, non-destructive way to leave interactive Create Mode.
        QShortcut(Qt.Key_Escape, self, activated=self._escape_create_mode)
    ...
    def closeEvent(self, event):
        # Stop GUI-side timers and interactive creation before Qt tears down
        # the plot.  Generation counters already make late worker results
        # harmless, so closing does not require a blocking wait on workers.
        self.map_canvas._lod_switch_timer.stop()
        if self.create_mode_btn.isChecked():
    ...
    def load_shapefiles_from_folder(self):
        ...
        root = Path(folder)
        # pathlib globbing can be case-sensitive on some platforms/filesystems.
        # Inspect the suffix explicitly so .SHP/.Shp files are included too.
        files = sorted(
    ...
    def load_shapefiles(self, files):
        ...
        # Give the user a concise explanation when a batch contains files that
        # could not be loaded, without interrupting every file with a dialog.
        if skipped_names:
            shown = skipped_names[:12]
            details = "\n".join(f"• {name}" for name in shown)
            if len(skipped_names) > len(shown):
                details += f"\n• ... and {len(skipped_names) - len(shown)} more"
            QMessageBox.information(
                self,
                "Batch Load Summary",
                summary + "\n\nSkipped files:\n" + details,
    ...
    def eventFilter(self, watched, event):
        if watched is self.layer_list.viewport() and event.type() == QEvent.MouseButtonPress:
            if event.button() == Qt.LeftButton:
                ...
                if item is not None:
                    ...
                    # QListWidget reserves the left part of the row for the
                    # check box.  Our layer icon (colour swatch + QC dot)
                    # follows it.  Keep a generous hit area around the
                    # colour swatch, but not the QC status dot.
                    if 20 <= x <= 38:
    ...
    def add_layer_to_list(self, layer_id: str, display_name: str, color_hex: str):
        ...
        item.setIcon(make_layer_icon(color_hex, "CHECKING..."))
    ...
    def update_layer_qc_status(self, layer_id: str, status: str):
        for index in range(self.layer_list.count()):
            ...
            item.setText(name)
            item.setIcon(make_layer_icon(color_hex, status))
    ...
    def show_layer_context_menu(self, pos):
        ...
        attribute_style_action = menu.addAction("Style by Attribute...")
        clear_attribute_style_action = menu.addAction("Clear Attribute Styling")
        menu.addSeparator()
        ...
        if action == zoom_action:
            ...
            self.change_item_color(item)
        elif action == attribute_style_action:
            layer_id = item.data(Qt.UserRole)
            if layer_id:
                self.map_canvas.show_attribute_style_dialog(layer_id)
        elif action == clear_attribute_style_action:
            layer_id = item.data(Qt.UserRole)
            if layer_id:
                self.map_canvas.clear_attribute_style(layer_id)
    ...
    def change_item_color(self, item: QListWidgetItem):
        ...
        if color.isValid():
            ...
            # Refresh the visible layer icon as well as the map graphics.
            # The QC status is kept unchanged; only the colour swatch changes.
            layer = self.map_canvas.loaded_layers.get(layer_id, {})
            qc_status = layer.get("qc_status", "CHECKING...")
            item.setIcon(make_layer_icon(color_hex, qc_status))
            item.setToolTip(f"Geometry QC: {qc_status}\nLayer color: {color_hex}")

    def _escape_create_mode(self):
        if self.create_mode_btn.isChecked():
            self.cancel_create_mode()
    ...
    def clear_canvas(self):
        # Keep the UI state consistent if the user clears the map while
        # interactively creating a feature.  The preview must not leave
        # Create Mode controls enabled after the canvas has been cleared.
        if self.create_mode_btn.isChecked():
            self.cancel_create_mode()
