"""
Great. Stage 7 — Feature Inspection is ready.

This stage adds the first connection between the map and the actual data behind it.

What's new

Click a feature on the map

A left-click near a visible feature will:

Identify the feature using a spatial index.
Highlight it in red.
Open a Feature Inspection window.
Show its geometry information.
Show all of its attribute values.


Performance consideration

For your large shapefiles, I deliberately didn't build the spatial index in the GUI thread.

It is prepared in the existing background worker queue, so loading remains responsive.
...
Load shapefile
      ...
      ├── Build LOD cache ────── background
      ├── Geometry QC ────────── background
      └── Feature index ──────── background

Once the index is ready, feature identification is very fast.

Also added

Right-click a layer → Clear Feature Selection

So you can remove the red highlight without affecting the layer itself.
"""
import sys
import time
