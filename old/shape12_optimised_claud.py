...
def _coords_with_nan_breaks(coords: np.ndarray, index: np.ndarray):
    """
    Vectorized: given stacked coordinates and a parallel array marking which
    source geometry each coordinate belongs to, insert a NaN row wherever the
    geometry index changes. Replaces a per-geometry Python loop.
    """
    if coords.size == 0:
        return np.empty(0, dtype=np.float64), np.empty(0, dtype=np.float64)

    break_positions = np.flatnonzero(np.diff(index)) + 1
    x = np.insert(coords[:, 0], break_positions, np.nan)
    y = np.insert(coords[:, 1], break_positions, np.nan)
    return x, y
...
class QCMapCanvas(pg.PlotWidget):
    ...
    def add_shapefile_layer(self, filepath: str, target_crs: str = None) -> bool:
        ...
        try:
            ...
            # 2. VECTORIZED LINE & POLYGON RENDERING (1 item per layer using NaN breaks)
            # No per-geometry Python loop: explode Multi* geometries (GEOS-vectorized),
            # pull all coordinates in one call, and insert NaN separators by index.
            ...
            if line_mask.any():
                lines = gdf.geometry[line_mask].explode(index_parts=False).dropna()
                coords, idx = shapely.get_coordinates(lines.values, return_index=True)
            ...
            if poly_mask.any():
                polys = gdf.geometry[poly_mask].explode(index_parts=False).dropna()
                # Exterior ring only, matching original behavior (holes ignored).
                rings = shapely.get_exterior_ring(polys.values)
            ...
            if x_parts:
                # NaN-separate the line group from the polygon group too.
                sep = np.array([np.nan])
                all_x = x_parts[0] if len(x_parts) == 1 else np.concatenate(
                    [x_parts[0], sep, x_parts[1]]
                )
                all_y = y_parts[0] if len(y_parts) == 1 else np.concatenate(
                    [y_parts[0], sep, y_parts[1]]
                )

                curve = pg.PlotCurveItem(
                    all_x, all_y,
                    pen=pg.mkPen(color=color, width=1.5),
                    connect='finite',   # breaks lines across NaNs
                    antialias=False,    # antialiasing is expensive on huge paths
                    skipFiniteCheck=True,  # we already control exactly where NaNs are
                )
