...
class QCMapCanvas(pg.PlotWidget):
    ...
    def add_shapefile_layer(self, filepath: str, target_crs: str = None) -> bool:
        ...
        try:
            ...
            # 2. Render LineStrings & MultiLineStrings
            line_mask = gdf.geometry.type.isin(['LineString', 'MultiLineString'])
            if line_mask.any():
                for geom in gdf[line_mask].geometry:
            ...
            # 3. Render Polygons & MultiPolygons
            poly_mask = gdf.geometry.type.isin(['Polygon', 'MultiPolygon'])
            if poly_mask.any():
                for geom in gdf[poly_mask].geometry:
