"""
Stage 9 adds: QC Report

There is now a QC Report button in the map toolbar.

It reports all currently loaded layers and includes:
...
Bounds
QC issues
Overall PASS / WARNING / ERROR status
Two export formats

HTML

A clean report suitable for opening in a browser or sending to someone.
...
The Excel export uses openpyxl.

You can also choose:

Export Both...

which creates both reports in one folder.
...
The report therefore gives you a quick project-level QC result as well as individual layer results.
"""
import sys
import time
...
class PipelineRouteCreator(QMainWindow):
    def __init__(self):
        ...
        report_action.triggered.connect(
            lambda: self.map_canvas.export_qc_report()
        )
