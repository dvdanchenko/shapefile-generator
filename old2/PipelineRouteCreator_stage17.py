...
A layer's normal colour remains unchanged, but you can now additionally style it from an attribute field.

Right-click a layer → Style by Attribute...

Numerical fields
Automatically detects numeric fields.
Applies a continuous viridis colour ramp.
Editable minimum / maximum values.
24 colour bins are used internally to keep rendering efficient on large datasets.
Null/non-finite values use the layer's normal colour.
Attempts to display a pyqtgraph colourbar labelled with the selected field.
Categorical fields
Automatically detects non-numeric fields.
Assigns distinct colours to categories.
Displays a map legend.
Limits the number of displayed categories to keep the map manageable.
Less frequent categories beyond the limit are grouped as <Other>.
Maximum category count can be selected in the styling dialog.
Important design choice

The attribute styling is independent of the basic layer colour.

So:

Layer colour = normal/default display colour
Attribute styling = optional data-driven display

Clearing attribute styling restores the original layer rendering and LOD behaviour.

Other things retained
Existing Active Layers controls.
Correct colour-swatch click behaviour via layer_list.viewport().
Context-menu colour selection.
Cached LOD rendering for normally styled layers.
Geometry QC.
Feature inspection.
Attribute table.
Create Mode.
...
The Stage 17 file and both existing support modules pass Python syntax compilation.

Test sequence

I'd test this in this order:

Load a shapefile containing a numeric field.
Right-click → Style by Attribute...
Select the numeric field → Apply.
Check that the map changes to the value-based ramp.
Change the minimum/maximum and apply again.
Check the colourbar.
Clear attribute styling and confirm the original layer colour returns.
Repeat with a categorical field.
Check the category legend.
Test visibility on/off while attribute styling is active.
Change the basic layer colour while attribute styling is active.
Remove the layer and confirm there are no leftover styled graphics.

If this passes, Stage 18 is the overlapping-feature selection system: clicking where several features/layers overlap will let you inspect and choose among the candidates rather than always selecting only the single nearest feature.
"""
import sys
import time
...
from PySide6.QtWidgets import (
    ...
    QTableView, QAbstractItemView, QLineEdit, QHeaderView, QSpinBox
)
...
class QCMapCanvas(pg.PlotWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Stage 17: attribute-driven map styling.
        # Styled graphics are grouped into bins/categories to avoid creating
        # one graphics item per feature on large layers.
    ...
    def clear_attribute_style(self, layer_id=None):
        ...
        for item in list(self._attribute_style_items) + list(self._attribute_style_aux_items):
            try:
                self.removeItem(item)
            except Exception:
                pass

        styled_layer_id = self._attribute_style_layer_id
        self._attribute_style_items = []
        self._attribute_style_aux_items = []
        self._attribute_style_layer_id = None
        self._attribute_style_field = None
...
<tbody>{''.join(table_rows)}</tbody>
    ...
    def set_layer_visibility(self, layer_id: str, visible: bool):
        ...
        styled = self._attribute_style_layer_id == layer_id and layer.get("attribute_style")
        for item in set(items):
            item.setVisible(visible and not styled)

        if styled:
            for item in self._attribute_style_items:
                item.setVisible(visible)
            for item in self._attribute_style_aux_items:
                try:
                    item.setVisible(visible)
                except Exception:
                    pass
