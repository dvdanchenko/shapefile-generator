"""
Stage 18 — Multiple / Overlapping Feature Selection

The map click behaviour is now:

One feature under the cursor: behaves exactly as before — highlight + Feature Inspection.
Multiple features under/near the cursor: opens a “Select Feature — Multiple Features at Click” dialog.
Each candidate gets its own tab, showing:
layer name
feature number
original index
geometry type
click distance
polygon containment information
geometry length/area
all attribute values
Switching tabs immediately highlights that candidate on the map.
Select This Feature confirms the feature and opens the normal Feature Inspection window.
Candidates are ordered by:
polygon containing the click
distance from click
layer order
feature order
If an unusually large number of features overlap, the dialog shows the 40 closest candidates rather than creating an unusable tab set.
Existing Create Mode behaviour is preserved.
Existing single-feature inspection is preserved.
_pick_feature() remains available for compatibility with existing code.

I also ran a Python syntax/compile check successfully.

Stage 18 file:

Download PipelineRouteCreator_stage18.py

I would test these cases before moving to Stage 19:

Two overlapping polygons → choose either one.
Two lines crossing → select either line.
Same geometry duplicated in two layers → choose the layer/feature you want.
Several layers with unrelated features → normal single-feature inspection still works.
Create Mode → clicking should still create points rather than open feature selection.
"""
