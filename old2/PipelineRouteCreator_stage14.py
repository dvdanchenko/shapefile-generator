"""
tage 14 — Polygon creation

Starting from the tested Stage 13 code, I added polygon creation without changing the existing Points/Line behaviour.

The geometry selector is now:
...
Polygon creation works as follows:

Enter coordinates in boundary order.
You do not need to repeat the first coordinate.
Minimum 3 distinct points required.
Polygon is automatically closed.
Zero-area polygons are rejected.
Self-intersecting/otherwise invalid polygons are detected and rejected.
The Shapely validity reason is shown to the user.
Area and perimeter are reported after creating the polygon.
The resulting polygon is saved as a normal shapefile and loaded immediately into the QC canvas.
Existing Points and Line workflows remain unchanged.
"""
import sys
import time
