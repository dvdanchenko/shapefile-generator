...
The Active Layers list now has a dedicated colour swatch:

Each layer shows:
colour square → layer colour
status dot → PASS / WARNING / ERROR / CHECKING
layer name
Left-click the colour square to open the colour picker.
The map and all cached LOD representations update immediately when the colour changes.
The tooltip now explains the colour/status indicators.
The existing right-click → Change Color... option remains as a fallback.
Layer visibility/check boxes and all existing Active Layers functionality are unchanged.

I also syntax-checked Stage 16 and both refactored support modules successfully.

Test

Please check:

Load several shapefiles with different colours.
Confirm the colour square is visible for every layer.
Click the colour square and choose another colour.
Confirm the layer changes colour immediately on the map.
Zoom/pan afterwards and confirm cached LODs retain the new colour.
Confirm PASS/WARNING/ERROR status indicators still appear.
Confirm checkbox visibility still works.
Confirm right-click → Change Color still works.
"""
import sys
import time
...
class PipelineRouteCreator(QMainWindow):
    ...
    def eventFilter(self, watched, event):
        if watched is self.layer_list and event.type() == QEvent.MouseButtonPress:
            if event.button() == Qt.LeftButton:
                item = self.layer_list.itemAt(event.position().toPoint())
                if item is not None:
